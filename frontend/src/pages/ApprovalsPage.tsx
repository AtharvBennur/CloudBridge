    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="border-border/70 shadow-sm">
          ...
          <CardContent>
            <div className="text-3xl font-semibold text-emerald-600">156</div>
        ...
        <Card className="border-border/70 shadow-sm">
          ...
          <CardContent>
            <div className="text-3xl font-semibold text-red-600">12</div>
      ...
      <Card className="border-border/70 shadow-sm">
        ...
        <CardContent>
          <div className="space-y-4">
            {[
              { type: "DROP_COLUMN", table: "orders", column: "legacy_id", risk: "CRITICAL", migration: "Production to DR", time: "2 hours ago" },
              { type: "ALTER_TABLE", table: "transactions", column: "amount", risk: "HIGH", migration: "Analytics to Warehouse", time: "4 hours ago" },
              { type: "DROP_TABLE", table: "temp_users", risk: "MODERATE", migration: "Legacy to Cloud", time: "6 hours ago" },
            ].map((approval, index) => (
              <div key={index} className="flex items-center justify-between p-4 border border-border/70 rounded-2xl bg-background/50 hover:bg-muted/20 transition">
                <div className="flex items-center gap-4">
                  <div className={`h-10 w-10 rounded-xl flex items-center justify-center ${
                    approval.risk === "CRITICAL" ? "bg-red-500/10 text-red-600" : 
                    approval.risk === "HIGH" ? "bg-orange-500/10 text-orange-600" : 
                    "bg-yellow-500/10 text-yellow-600"
                  }`}>
                    <AlertTriangle className="h-5 w-5" />
                  </div>
                  <div>
                    <h4 className="font-semibold">{approval.type.replace('_', ' ')}</h4>
                    <p className="text-sm text-muted-foreground">
                      {approval.table} • {approval.column ? `Column: ${approval.column}` : ''}
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">{approval.migration}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <Badge variant={approval.risk === "CRITICAL" ? "destructive" : approval.risk === "HIGH" ? "warning" : "secondary"}>
                    {approval.risk}
                  </Badge>

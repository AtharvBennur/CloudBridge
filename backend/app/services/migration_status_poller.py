...
class MigrationStatusPoller:
    ...
    def _poll_loop(self, ecs_task_id: int, app) -> None:
        """Background polling loop for a single ECS task."""
        # Push application context for the background thread
        with app.app_context():
            try:
                ...
                def _start_log_streaming_when_ready():
                    nonlocal log_thread
                    # Push app context for this thread as well
                    with app.app_context():
                        for _ in range(60):  # up to 5 minutes
                            ...
                            if task.log_stream_name:
                                log_thread = threading.Thread(
                                    ...
                                    args=(log_service, task, migration, app),
                                    daemon=True,
                                )
                                log_thread.start()
                                return
